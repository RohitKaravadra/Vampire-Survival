...
Vector2::Vector2(float value) { set(value, value); }

Vector2::Vector2(float _x, float _y) { set(_x, _y); }
...
Vector2 Vector2::to_int() { return Vector2(static_cast<int>(x), static_cast<int>(y)); }

float Vector2::distance(Vector2 v2) const { return (*this - v2).magnitude(); }

Vector2 Vector2::direction(Vector2& v2) const { return (v2 - *this).normalize(); }
...
Vector2 Vector2::operator-() { x = -x; y = -y; return *this; }
Vector2 Vector2::operator+(const Vector2& v2) const { return Vector2(x + v2.x, y + v2.y); }
Vector2 Vector2::operator-(const Vector2& v2) const { return Vector2(x - v2.x, y - v2.y); }
Vector2 Vector2::operator*(const float& value) const { return Vector2(x * value, y * value); }
Vector2 Vector2::operator/(const float& value) const { return Vector2(x / value, y / value); }
Vector2& Vector2::operator+=(const Vector2& v2) { this->x += v2.x; this->y += v2.y;	return *this; }
Vector2& Vector2::operator-=(const Vector2& v2) { this->x -= v2.x; this->y -= v2.y;	return *this; }
Vector2& Vector2::operator*=(const float& value) { this->x *= value; this->y *= value; return *this; }
Vector2& Vector2::operator/=(const float& value) { this->x /= value; this->y /= value;	return *this; }
...
// constructors
Rect::Rect() { set(Vector2(4), Vector2::zero); }

Rect::Rect(Vector2 _size, Vector2 _center) { set(_size, _center); }
...
Vector2 Rect::get_center() const { return Vector2(l + (r - l) / 2, t + (b - t) / 2); }

Vector2 Rect::get_topleft() const { return Vector2(l, t); }

Vector2 Rect::get_botmright() const { return Vector2(r, b); }
...
// converts given position into a grid position with given cell size
// works with square grid system only
...
// converts time into time format
// class can be used 
...
// generate a random number out of the given rect
